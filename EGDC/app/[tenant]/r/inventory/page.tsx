...
// Default column configuration for retailer inventory
const DEFAULT_RETAILER_COLUMNS: ColumnConfig[] = [
  ...
  { key: 'inventory_total', label: 'Total Stock', visible: true, category: 'inventory' },
  { key: 'shein', label: 'SHEIN', visible: false, category: 'platforms' },
  { key: 'meli', label: 'MercadoLibre', visible: true, category: 'platforms' },
  { key: 'shopify', label: 'Shopify', visible: true, category: 'platforms' },
...
export default function RetailerInventoryPage() {
  ...
  const [loadingText, setLoadingText] = useState('Cargando inventario del retailer...')
  ...
  const [columnConfig, setColumnConfig] = useState<ColumnConfig[]>(DEFAULT_RETAILER_COLUMNS)
  ...
  // Authentication and business type check
  useEffect(() => {
    ...
    console.log('🏪 Retailer Inventory - Tenant validation:', {
      userTenant,
      currentTenant,
      businessType,
      isValidTenantAccess,
    ...
    // Load inventory data for authenticated retailer
    loadInventoryData(1)
    loadProductCounts()
  ...
  const loadInventoryData = async (page: number = currentPage, search: string = '') => {
    try {
      setLoading(true)
      setLoadingText('Cargando datos del inventario para retailer...')
      ...
      console.error('Error loading retailer inventory data:', error)
      showToast(
  ...
  const handlePresetSelect = (preset: string) => {
    const presets = {
      ...
      financial: ['categoria', 'marca', 'modelo', 'costo', 'precio_shopify', 'precio_meli'],
      stock: ['categoria', 'marca', 'modelo', 'inv_egdc', 'inventory_total'],
      complete: 'all'
    }
        ...
        <div className="px-6 py-3 bg-white border-b border-gray-200">
          <div className="egdc-section-header">
            <h2 className="egdc-table-title">
              📦 Inventario EGDC Retailer ({totalItems} productos)
            </h2>
            <p className="text-gray-600 text-sm mt-1">
              Gestión de inventario para retailer - Vista completa con precios y stock
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 px-6 pb-1 overflow-hidden min-h-0">
          <ErrorBoundary
            ...
            onError={(error, errorInfo) => {
              console.error('Retailer inventory table error:', error, errorInfo)
            }}
            ...
            fallback={
              <div className="egdc-card p-8 text-center h-full flex flex-col justify-center border-red-200">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Error en la tabla de inventario</h3>
                <p className="text-gray-600 mb-4">
                  Hubo un problema mostrando la tabla de inventario del retailer. 
                </p>
                <button
